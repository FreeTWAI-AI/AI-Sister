...
for (const error of ['查詢已停止', new Error('查詢已停止')]) {
  ...
  for (const [name, content] of [['rows', stopped.rows()], ['outbound', stopped.el('[data-cli-outbound]').textContent], ['board', stopped.el('[data-cli-board]').textContent]]) {
    check(`stopped-expand-${typeof error}-${name}-keeps-reason`, content.includes('查詢已停止'));
    check(`stopped-expand-${typeof error}-${name}-no-retry-advice`, !content.includes('重新查詢'));
  }
}
const stoppedView = boot({cli_status_read: () => Promise.reject('查詢已停止'), cli_status_outbound: 3, usage_status_read: { stopped: true, config_readable: false }});
