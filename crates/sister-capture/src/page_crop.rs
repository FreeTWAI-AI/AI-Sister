...
//! stand in for the page while nodes are still uncounted.
...
///
/// `child_at_cap` is consulted only at the depth cap. Below the cap the walk
/// enqueues the children it can read, and a missing child read is the same
/// COM error as the end of a sibling list — this function does not pretend
/// those two are different. At the cap, `Some(false)` is the measured leaf
/// and `None` means the probe did not happen.
...
///
/// `next` returning `None` ends the chain. Callers that cannot tell a failed
/// sibling read from the real end both pass `None` here; this function does
/// not invent a third state for them.
pub(crate) fn sibling_chain<T>(
    mut first: Option<T>,
    mut next: impl FnMut(&T) -> Option<T>,
) -> Vec<T> {
    let mut out = Vec::new();
    while let Some(current) = first.take() {
        first = next(&current);
        out.push(current);
    }
    out
...
/// enqueued direct children, or the node cap refused a direct child. It is
/// not "the layer was never checked". The two counts compare equal either
/// way around; a direct-child refusal fails closed even when the counts match.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl DepthOneComplete {
    ...
    fn from_walk(
        ...
        Self {
            complete: direct_enqueued == depth_one_observed && !direct_child_blocked_by_node_cap,
        }
    }
...
impl<T> BreadthWalk<T> {
    pub(crate) fn seed(direct_children: impl IntoIterator<Item = T>) -> Self {
    ...
    pub(crate) fn enqueue_children(
        ...
        self.pending
            .extend(children.into_iter().map(|child| (child, child_depth)));
    }
...
mod tests {
    ...
    fn depth_one_left_unobserved() -> DepthOneComplete {
        DepthOneComplete::from_walk(2, 1, true)
    }

    fn seen_direct(count: usize) -> DepthOneComplete {
        DepthOneComplete::from_walk(count, count, false)
    }

    #[test]
    fn direct_children_include_every_sibling_not_only_the_first() {
        let children = sibling_chain(Some(1_u32), |n| (*n < 3).then_some(n + 1));
        assert_eq!(children, vec![1, 2, 3]);
        assert!(sibling_chain(None::<u32>, |_| Some(1)).is_empty());
    }
    ...
    fn an_unfinished_walk_does_not_crop_when_a_direct_child_was_blocked() {
        ...
        let blocked = choose_page_crop(
            ...
            Some(monitor()),
            false,
            DepthOneComplete::from_walk(3, 2, true),
        );
        ...
        let cap_flag = choose_page_crop(
            ...
            Some(monitor()),
            false,
            DepthOneComplete::from_walk(2, 2, true),
        );
        ...
        let short_count = choose_page_crop(
            ...
            Some(monitor()),
            false,
            DepthOneComplete::from_walk(3, 2, false),
        );
    ...
    fn depth_one_is_all_observed_before_deeper_nodes_spend_the_budget() {
        let mut walk = BreadthWalk::seed([0_u32, 1]);
            ...
            match frontier(walk.visited(), depth.get(), None) {
                ...
                Frontier::Open { descend } => {
                    ...
                    if descend && id == 1 && depth.is_direct_child() {
                        walk.enqueue_children(depth, 10..(10 + PAGE_WALK_NODE_CAP));
                    }
                }
            }
        }
    ...
    fn the_node_cap_leaves_a_direct_child_unobserved() {
        let mut walk = BreadthWalk::seed(0..(PAGE_WALK_NODE_CAP + 3));
        let mut nodes = Vec::new();
        while let Some((id, depth)) = walk.pop_front() {
            match frontier(walk.visited(), depth.get(), None) {
                Frontier::BudgetLeftNodes => {
    ...
    fn a_child_past_the_depth_cap_does_not_make_depth_one_incomplete() {
        let mut walk = BreadthWalk::seed([0_u32]);
        let mut last = None;
